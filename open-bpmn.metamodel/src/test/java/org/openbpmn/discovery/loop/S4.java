...
public class S4 {
	private static Logger logger = Logger.getLogger(S4.class.getName());
	...
	public void testInputProcess() throws BPMNModelException, CloneNotSupportedException {
		logger.info("s4.bpmn done: Start generating model");
		String path = "src/test/resources/discovery/loop/s4_results.bpmn";
...
//
		...
		boolean results = BPMNComparatorExecutor.execute(path, "src/main/resources/discovery/loop/s4.bpmn");
		if(!results){
			logger.warning("s4.bpmn: The two models are not equivalent");
		}else{
			logger.info("s4.bpmn done: The two models are equivalent");
		}
