...
public class S3 {
	...
	public void testInputProcess() throws BPMNModelException, CloneNotSupportedException {
		logger.info("s3.bpmn done: Start generating model");
		String path = "src/test/resources/discovery/loop/s3_results.bpmn";
...
//		System.out.println(relations);
		...
		boolean results = BPMNComparatorExecutor.execute(path, "src/main/resources/discovery/loop/s3.bpmn");
		if(!results){
			logger.warning("s3.bpmn: The two models are not equivalent");
		}else{
			logger.info("s3.bpmn done: The two models are equivalent");
		}
