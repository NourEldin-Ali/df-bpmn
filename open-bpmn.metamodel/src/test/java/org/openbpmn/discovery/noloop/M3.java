...
public class M3 {
	private static Logger logger = Logger.getLogger(M3.class.getName());
	...
	public void testInputProcess() throws BPMNModelException, CloneNotSupportedException {
		logger.info("M3.bpmn done: Start generating model");
		String path = "src/test/resources/discovery/noloop/m3_results.bpmn";
...
//		System.out.println(relations);
		...
		boolean results = BPMNComparatorExecutor.execute(path, "src/main/resources/discovery/noloop/M3.bpmn");
		if(!results){
			logger.warning("M3.bpmn: The two models are not equivalent");
		}else{
			logger.info("M3.bpmn done: The two models are equivalent");
		}
