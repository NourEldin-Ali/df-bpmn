...
public class M1 {
	private static Logger logger = Logger.getLogger(M1.class.getName());
	...
	public void testInputProcess() throws BPMNModelException, CloneNotSupportedException {
		logger.info("M1.bpmn done: Start generating model");
		String path = "src/test/resources/discovery/noloop/m1_results.bpmn";
...
//		System.out.println(relations);
		...
		boolean results = BPMNComparatorExecutor.execute(path, "src/main/resources/discovery/noloop/M1.bpmn");
		if(!results){
			logger.warning("M1.bpmn: The two models are not equivalent");
		}else{
			logger.info("M1.bpmn done: The two models are equivalent");
		}
