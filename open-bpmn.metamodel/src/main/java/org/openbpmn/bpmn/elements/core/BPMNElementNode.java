...
public abstract class BPMNElementNode extends BPMNElement {
	 ...
	 * BPMNElement. During cloning a element it may happen that invalid references
	 * are copied as child elements. This method can be called to clean up such
	 * references.
	 * 
	 */
	...
	public Set<SequenceFlow> getOutgoingSequenceFlows() {
		...
		Set<SequenceFlow> result = this.bpmnProcess.getSequenceFlows().stream()
				.filter(c -> c.sourceRef.equals(this.getId())).collect(Collectors.toSet());
		return result;
	...
	public Set<SequenceFlow> getIngoingSequenceFlows() {
		// filter all sequenceFlows with a sourceRef to this elementNode
		Set<SequenceFlow> result = this.bpmnProcess.getSequenceFlows().stream()
				.filter(c -> c.targetRef.equals(this.getId())).collect(Collectors.toSet());
		return result;
	...
	public Set<Association> getAssociations() {
		...
		Set<Association> result = this.bpmnProcess.getAssociations().stream()
