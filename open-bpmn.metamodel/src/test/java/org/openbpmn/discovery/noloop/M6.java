...
public class M6 {
	private static Logger logger = Logger.getLogger(M6.class.getName());
	...
	public void testInputProcess() throws BPMNModelException, CloneNotSupportedException {
		logger.info("M6.bpmn done: Start generating model");
		String path = "src/test/resources/discovery/noloop/m6_results.bpmn";
...
//		System.out.println(relations);
		...
		boolean results = BPMNComparatorExecutor.execute(path, "src/main/resources/discovery/noloop/M6.bpmn");
		if(!results){
			logger.warning("M6.bpmn: The two models are not equivalent");
		}else{
			logger.info("M6.bpmn done: The two models are equivalent");
		}
